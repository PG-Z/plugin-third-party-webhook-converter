...
import type {Webhook, WebhookGroup} from "@/types";
import yaml from "yaml";
import {useFileSystemAccess} from "@vueuse/core";
import {formatDatetime} from "@/utils/date";
import {useQueryClient} from "@tanstack/vue-query";
import {useRouteQuery} from "@vueuse/router";
import {useMapFetch, useWebhookGroupFetch} from "@/composables/use-map";
...
const selectedLink = ref<Webhook | undefined>();
...
const draggableLinks = ref<Webhook[]>();
...
const handleOpenCreateModal = (link: Webhook) => {
...
const onPriorityChange = async () => {
  try {
    const promises = draggableLinks.value?.map((link: Webhook, index) => {
      if (link.spec) {
        link.spec.priority = index;
      }
      return axiosInstance.put(
        `/apis/webhook.third.aiheiyo.top/v1alpha1/webhooks/${link.metadata.name}`,
        link
      );
    });
    if (promises) {
...
const handleDelete = (link: Webhook) => {
  Dialog.warning({
    title: "是否确认删除当前的地图标记？",
    ...
    onConfirm: async () => {
      try {
        const promises = selectedLinks.value.map((link) => {
          return axiosInstance.delete(`/apis/webhook.third.aiheiyo.top/v1alpha1/webhooks/${link}`);
        });
        if (promises) {
          await Promise.all(promises);
        }
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      const promises = parsed.map((link) => {
        return axiosInstance.post("/apis/webhook.third.aiheiyo.top/v1alpha1/webhooks", link);
      });
      if (promises) {
        await Promise.all(promises);
      }
    } else {
      await axiosInstance.post("/apis/webhook.third.aiheiyo.top/v1alpha1/webhooks", parsed);
    }
  } catch (e) {
    console.error(e);
  } finally {
...
async function handleMoveInBatch(group: WebhookGroup) {
  ...
  const requests = linksToUpdate.map((link) => {
    return axiosInstance.put<Webhook>(
      `/apis/webhook.third.aiheiyo.top/v1alpha1/webhooks/${link?.metadata.name}`,
      {
        ...link,
        spec: {
...
async function handleMove(link: Webhook, group: WebhookGroup) {
