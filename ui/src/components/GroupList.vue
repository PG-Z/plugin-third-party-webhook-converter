...
import type {WebhookGroup, WebhookList} from "@/types";
import { inject, ref, watch, type Ref } from "vue";
import Draggable from "vuedraggable";
import { axiosInstance } from "@halo-dev/api-client";
import { useWebhookGroupFetch } from "@/composables/use-map";
import cloneDeep from "lodash.clonedeep";

const groupQuery = inject<Ref<string>>("groupQuery", ref(""));

const groupEditingModal = ref(false);
const selectedGroup = ref<WebhookGroup>();

const { groups, isLoading, refetch } = useWebhookGroupFetch();
const draggableGroups = ref<WebhookGroup[]>();
...
const handleOpenEditingModal = (group?: WebhookGroup) => {
...
const onPriorityChange = async () => {
  try {
    const promises = draggableGroups.value?.map((group: WebhookGroup, index) => {
      if (group.spec) {
        group.spec.priority = index;
      }
      return axiosInstance.put(
        `/apis/webhook.third.aiheiyo.top/v1alpha1/webhookgroups/${group.metadata.name}`,
        group
      );
    });
    if (promises) {
